/**
 * Οθόνη συνδρομής — εμφανίζεται όταν ο λογαριασμός επαγγελματία είναι deactivated
 * ή όταν χρειάζεται ανανέωση πληρωμής (ίδια λογική πλάνων με Payment).
 */
...
import { db } from '../api';
import type { SubscriptionPlanId, Professional } from '../api/types';
...
export default function SubscriptionScreen() {
  ...
  const isDeactivated = pro?.accountStatus === 'deactivated';
  ...
  const activate = async () => {
    ...
    try {
      ...
      Alert.alert('Ενεργοποίηση', 'Η συνδρομή ενημερώθηκε.', [{ text: 'OK' }]);
    } catch (e) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.wrap}>
      <Text style={styles.title}>Συνδρομή</Text>
      {isDeactivated ? (
        <Text style={styles.sub}>
          Ο λογαριασμός σου είναι απενεργοποιημένος (deactivated). Επίλεξε πλάνο για να
          επανενεργοποιηθεί η πρόσβαση.
        </Text>
      ) : (
        <Text style={styles.sub}>
          Η δοκιμαστική περίοδος έληξε ή η συνδρομή χρειάζεται ανανέωση.
        </Text>
      )}
      {daysLeft != null && daysLeft > 0 ? (
        <Text style={styles.hint}>Απομένουν {daysLeft} ημέρες trial.</Text>
      ) : null}
      ...
      >
        {loading ? (
          ...
          <Text style={styles.primaryText}>Ενεργοποίηση συνδρομής (demo)</Text>
        )}
      </TouchableOpacity>
